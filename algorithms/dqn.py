...
def create_logic(action_dim, encoder, optimizer, gamma=0.9):
    target_model = mlp.MLP(output_dim=action_dim)
    q_model = mlp.MLP(output_dim=action_dim)
    return DQNLogic(q_model, target_model, encoder, optimizer, gamma)



class DQNLogic(BaseLogic):
    def __init__(self, q_model, target_model, encoder, optimizer, 
                 gamma=0.9,
                 epsilon_start=1.0, 
                 epsilon_min=0.01, 
                 epsilon_decay=0.995,
        ...
        self.model = q_model #Live network
    ...
    def train_step(self, state, action, reward, next_state, done):
        ...
        with tf.GradientTape() as tape:
            # Get current Q-values for the states
            current_q_values = self.model(state)
            
            # Extract the Q-value for the specific action taken
            ...
            # Loss: Mean Squared Error between predicted and target
