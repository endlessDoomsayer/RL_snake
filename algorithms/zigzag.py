...
def create_logic(state_shape, action_dim, n_boards, optimizer, gamma=0.9):
    return SafeHamiltonianLogic(n_boards, state_shape[0])
...
class SafeHamiltonianLogic:
    def __init__(self, n_boards, board_size):
        ...
        self.N = board_size - 2  # Playable area (e.g., 5 for 7x7)
    ...
    def _generate_cycle_map(self):
        ...
        if N % 2 == 0:
            ...
            # --- ODD GRID LOGIC ---
            # 1. Vertical Zig-Zag for Columns 1 and 2 (Up to Row N-1)
            for r in range(1, N):
                # Column 1
                if r % 2 == 1: amap[(r, 1)] = 0 # UP
                else: amap[(r, 1)] = 1         # RIGHT
                # Column 2
                if r % 2 == 0: amap[(r, 2)] = 0 # UP
                elif r > 1: amap[(r, 2)] = 3    # LEFT
            
            # 2. Top Edge (Row N)
            for c in range(1, N):
                amap[(N, c)] = 1 # RIGHT
                
            # 3. Right Edge (Column N)
            for r in range(2, N + 1):
                amap[(r, N)] = 2 # DOWN
            
            # 4. Internal Vertical Zig-Zag (Columns N-1 down to 3)
            # We fill these columns from the right to the left
            for c in range(N - 1, 2, -1):
                # Distance from the right edge determines direction
                # For N=5, Col 4 is even distance from N (1), Col 3 is odd (2)
                dist_from_right = (N - 1) - c
                
                if dist_from_right % 2 == 0:
                    # Upward Column
                    for r in range(1, N):
                        if r < N - 1: amap[(r, c)] = 0 # UP
                        else: amap[(r, c)] = 3         # LEFT
                else:
                    # Downward Column
                    for r in range(1, N):
                        if r > 1: amap[(r, c)] = 2     # DOWN
                        else: amap[(r, c)] = 3         # LEFT

            # 5. Bottom Connectors & Special Tiles
            amap[(1, N)] = 3 # Corner (1, N) moves Left
            amap[(1, 2)] = 3 # Tile (1, 2) moves Left to restart at (1,1)
    ...
    def get_action(self, state, training=False):
        ...
        for b in range(self.n_boards):
            ...
            # Special Maneuver for Odd Grids at (N-1, 1)
    ...
    def _bfs_to_start(self, board, start):
        ...
        while queue:
            ...
            for m_idx, (dr, dc) in moves.items():
                ...
                if not (1 <= nr <= self.N and 1 <= nc <= self.N):
                    continue
                # Logic: Treat Fruit (1) and Body (2) as Walls during search
        ...
        # If trapped, move anywhere safe to stall
        return 0
